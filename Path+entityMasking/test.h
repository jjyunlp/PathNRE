...
vector<double> test(int bags_id) {
	...
	// store the max score from all paths
	for(int i = 0; i < test_path[b[bags_id]].size(); i++)
	{
		...
		for(int j = 0; j < relationTotal; j++)
		{
			// In test, the path only calculate the abs betweent r0 and r1+r2
...
void* testMode(void *id ) 
{
	...
	{
		// Get score for each relation
		vector<double> score;
		score = test(ii);
		map<int,int> ok;
		ok.clear();
		// Only one relation be 1 in ok
		// traverse all sentences in this bag to get labeled relation tag
		for(int j = 0; j < bags_test[b[ii]].size();j++)
		{
			int i = bags_test[b[ii]][j];
			ok[testrelationList[i]] = 1;
		}
		// use pthread_mutex_lock to push back results to aa
		pthread_mutex_lock (&mutexx);
		// Each bag will be append relationTotal times
		// pair(1/0, score)
		// relationTotal-1 means not include last relaion "NA"
		// The size of aa will be bags_size * (relationTotal-1)
...
void test() {
	...
	vector<int> b_sum;
	b_sum.clear();
	// ok contains all valid(relation is valid) sentence in each bag
	// b and b_sum contain the bag name and sentences number in each bag
	// tot add all valid sentences
	for (map<string,vector<int> >:: iterator it = bags_test.begin(); it!=bags_test.end(); it++)
	{
		//How many relations in this bag
		//It should be only one relation tag
		map<int,int> ok;
		ok.clear();
		for (int k=0; k<it->second.size(); k++)
		{
			int i = it->second[k];
	//		cout << "relation num: " << testrelationList[i] << endl;
			// Only focus on None-NA classes(99:NA)
			if (testrelationList[i]<99)
				ok[testrelationList[i]]=1;
		}
		// it->first is a entity pair
	//	cout << it->first << "- ok.size: " << ok.size() << endl;
		tot+=ok.size();
		if (it->second.size()>0)
		{
	...
	// a contains all bag name in training data
	...
	// b_sum represents the number of sentences from 0->i
	for (int i=1; i<b_sum.size(); i++)
		b_sum[i] += b_sum[i-1];
	cout<< "total sentences: " << b_sum[b_sum.size()-1] << "\ntotal bags: " <<b_sum.size()<<endl;
	int now = 0;
	// ll_test: vector<int> hard to understand
	// something like divide bags for each thread
	...
	int output_flag = 0;
	float correct1 = 0;
	// aa.size == bags_num * relation_total

...
double train(int flag, float g_bar, int *sentence, int *trainPositionE1, int *trainPositionE2, int len, int e1, int e2, int r1, float &res, float &res1, float *matrixW1Dao, float *matrixB1Dao, float *r, float *matrixRelationDao,float *positionVecDaoE1, float *positionVecDaoE2, float*matrixW1PositionE1Dao, float*matrixW1PositionE2Dao,  float alpha) {
		int tip[dimensionC];
		...
		// During training, we know the relation (r1)
		...
		if (flag)
		{
			...
			for (int i = 0; i < dimensionC; i++) {
				...
				{
					g = f_r[r2]/sum*alpha;
					// max objective => min -loss
					// The derivative of Loss for W2 is -(1-f_r[r2]/sum)
					// if r2 != r1, what happens? => different derivate(p)
					// if r2==r1, g-=alpha do the (derivate=p-1)
					if (r2 == r1)
						g -= alpha;
					g = g*g_bar;
					g1 += g * matrixRelationDao[r2 * dimensionC + i] * (1 -  r[i] * r[i]);
					matrixRelation[r2 * dimensionC + i] -= g * r[i]; //r[i] is the input of this layer
					if (i==0)
			...
			// What are these? Since the E*E*G? But we divided each part ?
			// Myabe the L1 or L2?
			for (int i = 0; i < dimensionC; i++) {
	...
	float *positionVecDaoE1, float *positionVecDaoE2, float*matrixW1PositionE1Dao, float*matrixW1PositionE2Dao,  float alpha)
{
	...
	// Traverse all sentences in this bag(bags_id)
	for (int k=0; k<bags_train[b_train[bags_id]].size(); k++)//softmax on main sentence
	{
		int i = bags_train[b_train[bags_id]][k];	//sentence id
		// tmp: p(r|\theta,s) means the prob of relation r based on current weight
		...
		// This is the max multi-instance learning
		// Get E(h,r,t|s)
		if (tmp1<tmp)
		{
			tmp1 = tmp;
	...
	chain_id.push_back(tmp2);	// record which sentence get the max probability
...
//	bool hah = false;
	for(int j = 0; j < num; j++)	//softmax on peripheral path
	{
		...
		// The bag for p1 or p2 (Path=p1+p2) is replaced by the relation based bags
		// We use h->e in p1 to find the relation-based bag
		// So we will create relation-based bags first and then a entity-pair as key based relation map
		// When training, we random select N sentences from related relation-based bag as the representation 
		// for this entity-pair
		for(int l = 1; l<=2 ; l++)
		{
			int pair_id = a[l];	// The number in train_path.txt means pair id??
//			if (pair_id >= b_train.size()){
//				hah = true;
//				break;
//			}
//			cout<<"num: " << num << endl;
//			cout<<"b_train[bags_id]: "<<b_train[bags_id] << endl;
	//		cout<<"pair: " << b_train[a[1]] << "and " << b_train[a[2]]<< endl;
			tmp1 = -1e8;
			tmp2 = -1;
			// According to this pair id, we can get the e1+e2+string
//			cout << ">>>" << b_train.size() << " pair_id" << pair_id << endl;
//			cout << b_train[pair_id] << endl;
			string relation_key = entity_relation[b_train[pair_id]];
//			cout << "relation_key: " << relation_key << endl;


			// Get E(h->e,rA) and E(e->t, rB) by Big Bag
			for (int k=0; k<bags_relation_train[relation_key].size(); k++)//softmax on main sentence
			{
				int i = bags_relation_train[relation_key][k];
				double tmp = train(0,1.0,trainLists[i], trainPositionE1[i], trainPositionE2[i], trainLength[i], headList[i], tailList[i], relationList[i], res, res1, matrixW1Dao, matrixB1Dao, r, matrixRelationDao, positionVecDaoE1, positionVecDaoE2, matrixW1PositionE1Dao, matrixW1PositionE2Dao, alpha1);
				if (tmp1<tmp)
				{
					tmp1 = tmp;
					tmp2 = i;
				}
				// This bag may be very large, so I have to get a suitable solution
				if ( k>=30){
					break;
				}
			}
			...
			*/
			...
			// The max prob sentence for jth path of E(h, r_gold, e) and E(e, r_gold, t)
			chain_loss.push_back(tmp1);
...
//		}
	...
	// Get max path
	// G(h,r,t|P) = max_i G(h,r,t|p_i)  ..=E(h,rA,e)E(e,rB,t)p(r|rA,rB)
	int max_path_id = 0;
	double max_add = -1e8;
	for (int j = 0; j < num; j++)
	{
		// This is the G(h,r,t|p_i)
		// chain_relation_loss[0] = 1.0 was set before, so, start from j+1
	...
	double rt = 0;
	rt = rt + chain_loss[0];
	//L(h,r,t) = E(h,r,t|S)+ w*G(h,r,t|P)
	// if num>0, then there must be a path
	if (num>0)
		rt += (1-chain_loss[0])*w*chain_loss[2*max_path_id+1]*chain_loss[2*max_path_id+2]*(chain_relation_loss[max_path_id+1]);

	// These should be the updating
	// first argument in train() is flag[whether to update]
	
	// This update the E(h,r,t) (The information from direct sentence)
	// g_bar = 1*chain_loss[0]/rt  this loss occupy the total loss
	double nuissance = train(1,1*chain_loss[0]/rt,trainLists[chain_id[0]], trainPositionE1[chain_id[0]], trainPositionE2[chain_id[0]], trainLength[chain_id[0]], headList[chain_id[0]], tailList[chain_id[0]], relationList[chain_id[0]], res, res1, matrixW1Dao, matrixB1Dao, r, matrixRelationDao, positionVecDaoE1, positionVecDaoE2, matrixW1PositionE1Dao, matrixW1PositionE2Dao, alpha1);
	
	if (num>0)
	{
		// This update the G(h,r,t) (The information from relatin path)
		int j = max_path_id;
		//update the E(h,rA,e)
		nuissance = train(1,(1-chain_loss[0])*w*chain_loss[2*max_path_id+1]*chain_loss[2*max_path_id+2]*(chain_relation_loss[max_path_id+1])/rt,trainLists[chain_id[2*max_path_id+1]], trainPositionE1[chain_id[2*max_path_id+1]], trainPositionE2[chain_id[2*max_path_id+1]], trainLength[chain_id[2*max_path_id+1]], headList[chain_id[2*max_path_id+1]], tailList[chain_id[2*max_path_id+1]], relationList[chain_id[2*max_path_id+1]], res, res1, matrixW1Dao, matrixB1Dao, r, matrixRelationDao, positionVecDaoE1, positionVecDaoE2, matrixW1PositionE1Dao, matrixW1PositionE2Dao, alpha1);
		//update the E(e,rB,t)
		nuissance = train(1,(1-chain_loss[0])*w*chain_loss[2*max_path_id+1]*chain_loss[2*max_path_id+2]*(chain_relation_loss[max_path_id+1])/rt,trainLists[chain_id[2*max_path_id+2]], trainPositionE1[chain_id[2*max_path_id+2]], trainPositionE2[chain_id[2*max_path_id+2]], trainLength[chain_id[2*max_path_id+2]], headList[chain_id[2*max_path_id+2]], tailList[chain_id[2*max_path_id+2]], relationList[chain_id[2*max_path_id+2]], res, res1, matrixW1Dao, matrixB1Dao, r, matrixRelationDao, positionVecDaoE1, positionVecDaoE2, matrixW1PositionE1Dao, matrixW1PositionE2Dao, alpha1);
		// chain_id[0] is the sentence(id) which get the max prob(multi-sentence learning)
		int r0 = relationList[chain_id[0]];
		int r1 = relationList[chain_id[2*j+1]];
		int r2 = relationList[chain_id[2*j+2]];
		//update the p(r|rA,rB)
		//last argument means this loss occupies the part of total loss
...
void* trainMode(void *id ) {
	...
	}
	{
		for (int k1 = batch; k1 > 0; k1--)
		{
			...
			j = c_train[j];
			// random select a bags (j)
			score += path_cnn(j, matrixW1Dao, matrixB1Dao, r, matrixRelationDao, positionVecDaoE1, positionVecDaoE2, matrixW1PositionE1Dao, matrixW1PositionE2Dao, alpha1);
...
void train() {
	int tmp = 0;
	// b_train saves all e1+e2+relationName
	// c_train saves all bags' id
	b_train.clear();
	c_train.clear();
